...
- (void)updateLiveDataValue:(NSArray *)liveData {
    ...
    if (liveData.count >= 8) {
        NSString    *liveDataStr = [NSString stringWithFormat:@"Sp:%@ Hr:%@ ST:%@ DU:%@ accx:%@ accy:%@ accz:%@ breathRate:%@", liveData[0], liveData[1], liveData[2], liveData[3],liveData[4],liveData[5],liveData[6], liveData[7]];
