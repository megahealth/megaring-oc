...
- (void)updateLiveDataValue:(NSArray *)liveData {
    ...
    if (liveData.count >= 4) {
        NSString    *liveDataStr = [NSString stringWithFormat:@"Sp:%@ Hr:%@ ST:%@ DU:%@ accx:%@ accy:%@ accz:%@", liveData[0], liveData[1], liveData[2], liveData[3],liveData[4],liveData[5],liveData[6]];
