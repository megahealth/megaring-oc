...
### 此demo不支持在模拟器上运行，如在使用模拟器时遇到错误，请尝试切换到真机上运行。

## 更新日志
|版本|说明|时间|
|:-:|-|:-:|

|1.13.11| 修复连续升级固件时的崩溃问题 |2025/7/31|

|1.13.10| 兼容 6.0 固件版本 |2025/3/14|

|1.13.9| 修复在离手时监测报告时长解析错误的问题 |2024/8/21|
...
|1.13.4| 修复部分型号指环连接后monitorModeUpdated代理方法不调用的问题 |2024/3/20|
...
|1.12.9| 更新解析数据的算法（算法版本：12292),快速收取数据 |2023/10/31|
...
5. 调用 +[MRApi parseRespiratoryData:block:] 解析呼吸数据，生成呼吸数据报告;
6. 当生成HRV数据报告时 +[MRApi parseMonitorData:completion:] 解析数据后，生成HRV的报告,可以查看 (MRReport.h) 的属性说明。
7. HRV的说明：开启睡眠监测之后，当[手指与指环保持静止不动]至少28分钟，会产生HRV的数据。（测试的时候，不是很好测量，最好长时间进行测试，因为短时间测试手指晃动了可能就不会产生HRV数据了，建议带回家睡觉的时候开启睡眠监测，第二天开始收数据）。

8.   ECG的说明: 开启血压检测完成之后，解析数据得到ECG数据 [开启血压监测，测量完成之后查看ecg的UI绘制详情]。
